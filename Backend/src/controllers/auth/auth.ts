...
export const loginAdmin = async (req: Request, res: Response) => {
    try {
        ...
        // 4. Tạo Token
        const accessToken = jwt.sign(
            { email: currentAdmin.email, adminId: currentAdmin.id, role: currentAdmin.role },
            process.env.JWT_ACCESS_SECRET as string,
            { expiresIn: '1h' }
        );
    ...
    try {
        ...
        try {
            ...
            if (decoded.userId) {
                ...
                const newAccessToken = jwt.sign(
                    { email: decoded.email, adminId: decoded.adminId, role: decoded.role },
                    process.env.JWT_ACCESS_SECRET as string,
                    { expiresIn: '1h' }
                );
...
export const logoutAdmin = async (req: Request, res: Response) => {
    try {
        ...
        if (currentRefreshToken) {
            await prisma.admin_refresh_tokens.delete({
                where: { token: currentRefreshToken }
            }).catch(() => {});
        }
...
// ==========================================
export const handleVerifyOtpAndResetPasswordUser = async (req: Request, res: Response) => {
    try {
...
// ==========================================
export const handleVerifyOtpAndResetPasswordAdmin = async (req: Request, res: Response) => {
    try {
        const { email, otpVerify, newPassword, confirmNewPassword } = req.body;
